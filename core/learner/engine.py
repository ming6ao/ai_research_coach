...
from core.db import learner_db_url
from core.task_decomposer import TaskDecomposer
from core.learner.container import build_container
from core.learner.domain.assessment import AssessmentTarget, AssessmentTask, TargetRole, TaskType
...
from core.learner.storage.database import Base, create_session_factory
...
class LearnerEngine:
    ...
    def _session(self) -> Session:
        if not self._schema_ready:
            # Idempotent create; the runtime path (also used by core.db.create_schema).
...
def pick_next_task(
    ...
    Hybrid selection (see docs/refactor-plan.md §7):
      ...
      3. EIG bank picker — ``core.picker.next_task(session)``.
    ...
    from core.session import task_view
...
# CLI inspector
#
#   python -m core.learner.engine <candidate>     print a candidate's snapshot
#   python -m core.learner.engine --demo           run a canned learner through
#                                                   the full loop and print it
#   python -m core.learner.engine --db <url> ...   override the learner DB
# ---------------------------------------------------------------------------
