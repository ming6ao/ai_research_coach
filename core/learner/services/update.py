"""LearnerUpdateService: applies evidence to the learner model.

The update math lives in ``domain.update.UpdateEngine`` (pure + testable);
this service only wires persistence and guarantees references exist. There is
no audit table: learner state is derived from append-only ``evidence``, so the
history is always recomputable.
"""
