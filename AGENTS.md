...
- **Add question**: `POST /api/v1/tasks` with `prompt`, required `tags` (`{primary: <leaf skill>, secondary: [...]}`), optional `scaffold`/`difficulty`/`parts`/`task_type`/version fields; or `initial_question` on `POST /api/v1/sessions` (auto-tagged by one combined LLM call — rejected if it cannot categorize); or start a session with a random question in one domain/area/skill via `node` on `POST /api/v1/sessions`
