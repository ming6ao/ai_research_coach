...
| Backfill per-step starter code (dry-run; `recover` redistributes a legacy task-level scaffold, `generate` writes LLM stubs; `--apply` backs up first) | `python -m coach.scaffold_backfill audit\|recover\|generate [--apply]` |
