...
| Reset activity/progress (keeps users/auth + task bank; `POST /admin/reset`; add `?wipe_tasks=true` to also drop the bank; legacy `source='generated'` rows are wiped with the sessions) | via the API — no CLI |
...
| Drop un-answered sessions (dry-run; `--apply` backs up first) | `python -m coach.migrate sessions [--apply]` |
| Audit bank scaffold hygiene (read-only; missing starter code, leaked internals/instance state) | `python -m coach.migrate hygiene` |
...
- **Database is the source of truth for tasks**: the `tasks` table is the only question bank — there is no code catalog. Tasks are authored in the DB via `POST /api/v1/tasks` or the curator UI ("My questions"). `create_schema()` (startup) creates/migrates tables but never writes tasks; `reset_database()` (`POST /admin/reset`) wipes activity/progress (`session_steps`, `user_skill_beliefs`, `active_sessions`) while preserving users/auth **and the task bank**; pass `?wipe_tasks=true` to also delete every task (used when re-authoring against a new taxonomy). Every task is **step-by-step**: `parts: [{key, prompt, tags, max_score, difficulty, scaffold, scaffolds?, pass_score?}]` are delivered one at a time (every submission advances to the next step, regardless of the score), with the candidate's code carried forward. A task declares `languages: [...]` (ordered; the first is the default and mirrors `language`): a single-language task behaves exactly as before, while a **multi-language** task (e.g. Python + C++) stores per-step `scaffolds: {language: code}` and requires starter code for **every** declared language — `validate_parts` rejects a missing one (422) on both `POST` and `PATCH`. The candidate picks the language (locked after the first submission), `POST /api/v1/sessions/{id}/answers` accepts an optional `language`, prior code is carried forward per language, and the judge uses the chosen language. Every task has **one or more parts**; a single-step question is a one-part task (there is no partless request). There is no single-submission mode and no version chains — `delivery` is always `'phased'`.
- **Tag everything (3-level taxonomy)**: every task — user, admin, or generated — carries `tags: {primary: <leaf skill>, secondary: [0-2 leaf skills]}` + `task_type` (`implement|apply|debug|design|analyze`); every part carries its own tags too. `coach/taxonomy.py` is the single source of truth: a 3-level tree `domain → area → skill` (3 domains / 18 areas / 110 built-in leaf skills), with `TAXONOMY`, `DOMAINS`, `AREAS`, `LEAF_NODES`, `NODE_PARENT`/`NODE_LEVEL`, `ancestors`, `domain_of`/`area_of`, `ALIASES`, `validate`, `format_vocabulary`. Domains/areas are fixed in code; leaf skills are **extensible**: a signed-in curator can add one under an existing area (`POST /api/v1/taxonomy/skills`, stored in the `custom_skills` table by `coach/custom_skills.py`), and `register_custom_skill` merges it into the live `TAXONOMY`/`LEAF_NODES`/`NODE_*` maps so tagging, beliefs, mastery, and the picker treat it like a built-in. Unknown tags, a missing primary, a non-leaf primary, or a skill under an unknown area are rejected (422). Tags are **required** — there is no permissive fallback (the no-key categorization path returns no tags and creation is rejected so the curator/admin must choose). Only primary leaf skills feed the belief system; secondary tags count for coverage/diversity only.
...
- **Hybrid question selection**: `coach/selection.py:pick_next_task` — (1) pending generated task, (2) **active step task** (`_active_step_task`: a started-but-unfinished task continues on its next step after each submission; its view carries the candidate's prior code as `previous_code`), (3) judge-driven follow-up (`coach/remediation.py`: a simpler task drilled from the judge's misconception/feedback text, difficulty tuned to ~80% P(solve) via `coach/solvability.py`), (4) EIG bank picker with small per-level exploration bonuses (`coach/picker.py`: `0.002·1/(1+domain_attempts) + 0.003·1/(1+area_attempts) + 0.001·1/(1+skill_attempts)`, minus a soft `-0.010` same-area penalty; generated tasks are excluded from the bank), (5) scope-widening challenge (least-covered skill). Branches (3) and (5) are LLM **write** effects that append to the session, so read/idempotent-replay callers (`GET /api/v1/sessions/{id}`, `_replay_response`) pass `allow_generation=False` — a GET never mints a task or spends an LLM call, it only surfaces a task already injected into the session. An optional `node` param restricts the bank branch to tasks under one domain/area/skill; a `sample_top_n` picks uniformly among the top-N EIG candidates
...
- **Step-by-step tasks**: a task's `parts` are delivered one at a time and scored **per step** by the judge (`coach/judge.py` returns `parts: [{key, score, rationale}]`). The judge scores only the active step; **every submission advances `session.task_progress` to the next step regardless of the score** (there is no pass gate or per-step retry), and the candidate's prior code is passed back as `previous_code`. Each step's primary skill updates the belief system (that skill plus its area/domain ancestors) at the step's own difficulty; the global belief updates once per submission with that step's fraction. A single-step task is one part. `coach/config.py` still derives each step's `pass_score` (`PHASE_PASS_FRACTION`) as authored metadata, but it no longer gates delivery; `PHASE_MAX_ATTEMPTS` is retained for backward compatibility only. There are **no hints** (`coach/hints.py` was removed).
...
- **Teaching pause**: After a submit the UI does **not** auto-advance. The store holds `pendingTask`; `ChatView` shows the coaching + per-part results and the candidate clicks `Next step` (the `advance()` store action) before the next step/task renders
...
- **Persistence**: single SQLite file `data/coach.db` (gitignored) with 7 tables (`users`, `auth_tokens`, `active_sessions`, `oauth_states`, `tasks`, `session_steps`, `user_skill_beliefs`). `coach/db.py` is the single connection module; `create_schema()` drops removed columns/tables (incl. `hints_json`/`cluster_id`/`followups_json`/`version_index`/`depends_on_task_id`/`version_root_id`, plus the retired `trajectory_shares` table and `session_steps.inherited`), adds `parts_json`/`delivery` plus `active_sessions.title`/`summary`, migrates beliefs to one row per `(candidate, level, key)` (`level` ∈ global/domain/area/skill) with a unique index (`uq_user_skill_beliefs`), and backfills `session_steps` from legacy JSON blobs. `session_steps` (`coach/steps.py`) holds one RL-shaped row per scored answer (`answer_for_task` fetches a task's submitted code). Candidate identity is resolved by `backend/dependencies.py:resolve_candidate` (signed-in email or stable `X-Guest-Id` per browser). **Session persistence is deferred**: `SessionState.create` only stages a draft in a bounded in-memory buffer; the row is upserted on the first scored answer (`save(..., persist=True)`), so abandoned "Start practice" clicks are never stored or listed — self-answered drafts drop on restart. `python -m coach.migrate sessions [--apply]` purges any legacy un-answered rows (and their explanations)
- **Models**: `EVAL_MODEL` (judge/coach + decomposition) defaults to `gemini-3.5-flash-lite`

## Extending Without Code Changes

- **Add question**: `POST /api/v1/tasks` with `parts` (**required, ≥1 step**; each step: `prompt`/`tags`/`max_score`/`difficulty`/**required** `scaffold`, or `scaffolds: {lang: code}` + task-level `languages` for a multi-language task), required `tags` (`{primary: <leaf skill>, secondary: [...]}`), optional `task_type`; a single-step question is a one-part task; or start a session with a random question in one domain/area/skill via `node` on `POST /api/v1/sessions`. **Quick authoring**: `POST /api/v1/tasks/draft` (signed-in, never persisted) turns curator step prompts (≤5) into a fully-populated task body — keys, `max_score`/`difficulty`/`pass_score`, taxonomy tags, per-step scaffolds, `task_type`, `context_notes` — or revises one via `{draft, instruction}`; `coach/task_decomposer.py:TaskDecomposer.draft_task` makes one structured call and degrades to a deterministic fallback (empty tags, empty scaffolds) without an API key. **New skill**: `POST /api/v1/taxonomy/skills {skill, area}` (signed-in) registers a custom leaf skill under an existing area and returns `{skill, area, domain}`; `GET /api/v1/taxonomy/skills` lists them; `DELETE /api/v1/taxonomy/skills/{skill}` removes one. `POST /admin/reset?wipe_tasks=true` also drops them.
...
| `code` (step-by-step) | `parts` each with `prompt`/`tags`/`max_score`/`difficulty`/`scaffold` + optional `pass_score` | Judge scores the active step only; every submission advances to the next step regardless of the score, carrying prior code as `previous_code` |

Every task carries `tags: {primary, secondary[]}` (leaf skills from `coach/taxonomy.py`; unknown/missing/non-leaf → 422) and a `task_type`; every step carries its own `tags`. Delivery is always step-by-step (`delivery='phased'`, per-step scaffolds and `pass_score`); **every task has at least one part** (a single-step question is a one-part task). Task-level `difficulty`/`max_score` are derived from the steps, `context_notes` is LLM-generated, and `tags.secondary` is auto-derived from the step primaries when omitted. The task-level `prompt` is also **derived** — always the first step's prompt (`coach/tasks.py:_task_prompt`), never authored; `POST`/`PATCH` no longer accept a top-level `prompt` or `scaffold`, and `create_schema()` wraps any legacy partless row into a single part before dropping the legacy task-level `prompt`/`scaffold` columns. `python -m coach.migrate hygiene` (read-only) audits the bank for missing starter code and scaffolds that leak private members/instance state. Every step requires its own scaffold: `validate_parts` rejects a task/step missing one (422) — for a multi-language task, one per declared language (`part.scaffold` mirrors the default while `part.scaffolds` holds every language). `coach/scaffold_validation.py:validate_scaffold` is the runtime stub validator used by the draft endpoint (rejecting implementation bodies, leaked instance state, invalid syntax). For session-only/generated tasks that never touch the bank, `coach.session._compose_step_scaffold` derives a best-effort stub from the prompt and never names a function after the internal step key.
...
- After a submit, the picked task is returned as `next_task` but held back by the UI until the candidate reviews the coaching and clicks **Next step** — the system never auto-advances. A `next_task: null` after the last question means the candidate is done; the frontend then returns to the merged home page, which shows the candidate's persisted progress (via `GET /api/v1/me/overview`).
...
- Follow-ups are judge-driven: `coach/remediation.py` drills the judge's gap text; generated tasks inherit the root task's tags. Generated drills/challenges are **session-only** (`plan_followup`/`plan_challenge` append to `session.tasks` + `generated_task_ids`, never to the `tasks` table), so they can not resurface as bank questions or orphan when the session goes away; `source='generated'` rows are legacy and dropped by `POST /admin/reset` and the per-candidate/guest wipe paths.
...
- Chat-style UI: `ChatView` in `frontend/src/components/Chat/` renders the session as coach/user bubbles; the active task embeds Monaco via `CodeEditor` (initialized from `previous_code ?? scaffolds[language] ?? scaffold`; a multi-language task shows a language picker that locks after the first submit); the prompt bubble lists the active step's question; submitted results render the judge's coaching (`CoachingBubble`: verdict chip + misconception + numbered steps with code examples). There are no hints
- Merged home page: `frontend/src/components/Home/HomeView.tsx` is the landing/progress page in one — greeting, a primary `Start practice` button (random mixed question), overall **mastery %** + a clickable domain→area→skill drill-down (every domain/area/skill starts a session with a question under that node via `{node}`), and the Recent-sessions list. The drill-down is **dynamic**: it only renders nodes that have a visible bank task, computed from `task_counts` in `GET /api/v1/me/overview` (`backend/v1/users.py:_task_counts`, counted from the task-level **primary** tag + ancestors only — a secondary-only node stays hidden because the picker can't serve a task for it). A domain/area/skill that has no associated task does not appear until a task tagged with it is created; the taxonomy vocabulary itself (`GET /api/v1/taxonomy`) stays complete so curators can author the first task for a new node. A `{node}` session start is **bank-only** (`allow_generation=False`): if no bank task's primary is under the node it returns 404 instead of minting a generated challenge. The free-text question `Composer` was **removed** from the home page — learners start from the practice button or an area. It loads its snapshot from `GET /api/v1/me/overview`. There is no separate progress screen; finishing a session returns here (beliefs persist server-side), and past sessions are reviewed by opening them from Recent sessions (full chat history with coaching). Each persisted session carries an LLM-generated `title` + `summary` (created on the first scored answer, deterministic fallback without an API key); the Recent-sessions row shows the title (timestamp below it) and the summary is its hover tooltip (`title` attribute). Labels are plain mastery percentages (`mastery.<level>.score`), not the internal confidence.
...
- Curator UI: `frontend/src/components/Curator/` — any signed-in user can author/manage their own questions via "My questions" in the header (`CuratorView` list + `TaskEditor` with a learner-view `TaskPreview`). The editor is **simple, step-by-step only** — no advanced mode and no delivery toggle: task `prompt`, one or more `languages` (the first is the default), a task-level primary tag, and an ordered list of steps (key, `prompt`, `max_score`, `difficulty`, primary tag, **required** scaffold) with per-step starter code. A multi-language question exposes one starter-code tab per selected language and requires each of them (`findStepDraftError` blocks Create until all are filled). Inline validation catches empty/duplicate step keys and blocks Create when a step lacks starter code (the server also rejects a missing scaffold with 422), and a non-blocking starter-code check warns when a scaffold exposes a `private:`/member/`self.x` layout. There is no overview field — the task-level prompt is derived from the first step. `QuestionBubble` renders only the active step's prompt (in the prominent "question" markdown style) and never shows a step key; the curator edits that step prompt inline while the key lives in the hidden step-details panel. Task ids and step keys are not displayed anywhere in the UI (curator notices use plain wording). New questions default public (toggleable `is_public`); `GET /api/v1/me/tasks` lists the owner's tasks with attempt counts, and `GET /api/v1/taxonomy` (public, unlike `/admin/taxonomy`) feeds the skill pickers. The step skill pickers are **open comboboxes** (`SkillCombobox` in `Editable.tsx`): type to filter the built-in/custom vocabulary, or type a missing name and pick an area to register it inline via `POST /api/v1/taxonomy/skills` (the editor then reloads the taxonomy so the new skill is a normal option). Domains and areas remain fixed; only leaf skills are extensible. The new-task editor also carries an **AI assistant** section: paste one step prompt per block (blank line or `---` separated, ≤5) and **Fill in details** calls `POST /api/v1/tasks/draft` to populate the form; a **Refine** field sends the current (possibly hand-edited) draft plus a plain-English instruction back to the same endpoint — the LLM proposes, the curator reviews and presses Create. A draft never synthesizes starter code: a step whose scaffold the model omitted (or produced invalid) stays empty and **Create question** is blocked (editor focuses the step) until scaffold is added. `context_notes` is editable in the settings panel.
