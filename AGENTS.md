...
- **Builtin question bank**: `coach/seed_bank.py` ships a **code-block** catalog (`SEED_CATALOG`): 13 blocks + 3 versioned chains covering every family and fine tag via per-part tags. A block is one task-level `scaffold` over related functions (`parts: [{key, prompt, tags, max_score, difficulty}]`); version successors are separate entries linked by `depends_on_task_id`/`version_root_id` (`version_index=2`). `seed_question_bank()` runs from `create_schema()` (startup), is hermetic (no LLM/network), uses a single batched `INSERT OR IGNORE`, and deletes stale `source='seed'` rows not in the catalog (no manual `--reset`). Human-edited seeds are never overwritten. `coverage_report()` (admin UI) + `--fill-gaps` mint tasks for uncovered/lowest-coverage tags as `source="seed_llm"`.
- **Tag everything**: every task — seed, user, or generated — carries `tags: {primary: <fine tag>, secondary: [0-2 fine tags]}` + `task_type` (`implement|apply|debug|design|analyze`); every part carries its own tags too. `coach/taxonomy.py` is the single source of truth (11 families / 46 fine tags, `ALIASES`, `validate`, `family_of`); unknown tags are rejected (422). Only primary tags (block or part) feed the belief system; secondary tags count for coverage/diversity only.
