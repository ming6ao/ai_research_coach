...
Every task carries `tags: {primary, secondary[]}` (leaf skills from `coach/taxonomy.py`; unknown/missing/non-leaf → 422) and a `task_type`. Code blocks additionally carry per-part `tags`. Multi-step tasks use `delivery='phased'` (per-part scaffolds and `pass_score`).
