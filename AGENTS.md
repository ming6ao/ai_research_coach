...
| `code` (step-by-step) | `parts` each with `prompt`/`tags`/`max_score`/`difficulty`/`scaffold` + optional `pass_score` | Judge scores the active step only; every submission advances to the next step regardless of the score, which starts from its own scaffold |
