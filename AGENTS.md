...
| `code` (step-by-step) | `parts` each with `prompt`/`tags`/`max_score`/`difficulty`/`scaffold` + optional `pass_score` | Judge scores the active step only; pass (`≥ pass_score`) or `PHASE_MAX_ATTEMPTS` advances, carrying prior code as `previous_code` |
