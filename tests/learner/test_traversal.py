...
from learner.types import EdgeType
...
class TestDirect:
    def test_direct_prerequisites(self, service):
        ...
        assert [x.name for x in service.direct_prerequisites(w.id)] == ["probability"]
        assert [x.name for x in service.direct_prerequisites(n.id)] == ["weighted_distribution"]
    ...
    def test_requires_edge_marks_source_as_prerequisite(self, service):
        ...
        assert [x.name for x in service.direct_prerequisites(n.id)] == ["probability"]
        assert [x.name for x in service.direct_dependents(p.id)] == ["normalize_weights"]

    def test_direct_dependents(self, service):
        p, w, n = _build_chain(service)
        assert [x.name for x in service.direct_dependents(p.id)] == ["weighted_distribution"]
        assert [x.name for x in service.direct_dependents(w.id)] == ["normalize_weights"]
    ...
    def test_neighbors_all_edge_types(self, service):
        ...
        assert [x.name for x in nbrs] == ["weighted_distribution"]
        assert [x.name for x in service.neighbors(w.id)] == ["probability"]


class TestDepth:
    def test_descendants(self, service):
        p, w, n = _build_chain(service)
        assert [x.name for x in service.descendants(p.id, max_depth=1)] == ["weighted_distribution"]
        assert [x.name for x in service.descendants(p.id, max_depth=2)] == [
            "normalize_weights",
            "weighted_distribution",
        ]
        assert [x.name for x in service.descendants(p.id)] == [
            "normalize_weights",
            "weighted_distribution",
        ]
    ...
    def test_ancestors(self, service):
        p, w, n = _build_chain(service)
        assert [x.name for x in service.ancestors(n.id, max_depth=1)] == ["weighted_distribution"]
        assert [x.name for x in service.ancestors(n.id, max_depth=2)] == [
            "probability",
            "weighted_distribution",
        ]
        assert [x.name for x in service.ancestors(n.id)] == [
            "probability",
            "weighted_distribution",
        ]
    ...
    def test_descendants_respect_max_depth_none(self, service):
        ...
        assert [x.name for x in service.descendants(a.id, max_depth=2)] == ["b", "c"]
        assert [x.name for x in service.descendants(a.id)] == ["b", "c", "d"]
    ...
    def test_cycle_is_handled(self, service):
        ...
        assert [x.name for x in service.descendants(a.id)] == ["b"]
        assert [x.name for x in service.ancestors(b.id)] == ["a"]

    def test_related_nodes_via_service(self, service):
        p, w, n = _build_chain(service)
        assert {x.name for x in service.get_related_nodes(w.id)} == {
            "probability",
            "normalize_weights",
        }